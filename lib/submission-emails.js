...
const { brandHeader, emailDocument } = require("./email-brand");
...
// Same idea for the film type. The team notification used to print the raw
// value, which was readable while it was just "feature"/"short" — it stops
// being readable once a tier is called `short_under_30`, and the tier is what
// decides the price, so it is worth spelling out.
const FILM_AR = {
  feature: "روائي طويل (حتى 120 صفحة)",
  short: "قصير (من 30 إلى 50 صفحة)",
  short_under_30: "قصير (أقل من 30 صفحة)",
  treatment_feature: "معالجة — فيلم طويل (حتى 15 صفحة)",
  treatment_short: "معالجة — فيلم قصير (حتى 5 صفحات)",
};
...
async function sendNotification(row) {
  ...
  const rows = [
    ...
    ["النوع", row.genre],
    ["نوع الفيلم", FILM_AR[row.film_type] || row.film_type],
    ["المسودة", row.draft],
    ["المدة", row.duration || "—"],
    ["الثيم", row.theme || "—"],
    ["Logline", row.logline || "—"],
  ...
  const html = emailDocument(notificationEmail(row, rows));
...
async function sendConfirmation(row) {
  ...
  const html = emailDocument(confirmationEmail(row));
...
// Launch-promotion welcome. Sent INSTEAD OF the payment confirmation for the
// submissions granted free at launch (`is_promo`), so a writer who paid nothing
// is never told "تم تأكيد الدفع". Same shell as the other writer emails.
//
// It deliberately says nothing about price, invoices or refunds: the grant is a
// gift, and mentioning money would only raise the question of what was paid.
function promoEmail(row) {
  var esc = escapeHtml;
  var title = row.title_ar || row.title_en || "";
  var name = (row.writer || "").toString().trim();

  var arHi = name ? "مرحبًا " + esc(name) + "،" : "مرحبًا، عزيزي/تي";
  var enHi = name ? "Dear " + esc(name) + "," : "Dear writer,";
  ...
  var bodyStyle = "margin:0 auto;max-width:440px;font-size:15px;line-height:1.9;color:#4a453f;";
  var noteStyle = "margin:22px auto 0;max-width:440px;font-size:13.5px;line-height:1.8;color:#8a8178;";
      ...
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f5f1e9;"><tr><td align="center">' +
        '<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;">' +
          '<tr><td style="padding:46px 44px;text-align:center;">' +
            ...
            '<h1 style="margin:0 0 14px;font-size:25px;line-height:1.3;color:#15110f;font-weight:700;">تم استلام طلب تقييمك الاستثنائي</h1>' +

            titleLine +

            '<p dir="rtl" style="' + bodyStyle + '">' +
              arHi +
              "<br>تم استلام طلب تقييمك الاستثنائي بمناسبة إطلاق Scene One. يسعدنا أن يكون نصك من بين الأعمال المختارة لهذه المبادرة الخاصة." +
              "<br><br>سيعمل فريقنا على مراجعة عملك بعناية ضمن تجربة تقييم احترافية تهدف إلى تسليط الضوء على النصوص الواعدة وإبراز حضورها في الصناعة." +
              "<br><br>شكرًا لثقتك، ونتطلع لاكتشاف قصتك." +
            "</p>" +
            ...
            '<p dir="ltr" style="' + bodyStyle + '">' +
              enHi +
              "<br>Your exceptional evaluation request has been received as part of the launch of Scene One. We are pleased that your script is among the selected works for this special initiative." +
              "<br><br>Our team will carefully review your work as part of a professional evaluation experience aimed at highlighting promising scripts and bringing them into industry attention." +
              "<br><br>Thank you for your trust, and we look forward to discovering your story." +
            "</p>" +

            '<p dir="rtl" style="' + noteStyle + '">' +
              "سنرسل إليك تقرير التقييم عبر بريدك الإلكتروني فور اكتماله واعتماده." +
              "<br><span dir=\"ltr\">We\'ll email you the coverage report once it is complete and approved.</span>" +
            "</p>" +
...
// Sends the launch-promotion welcome to one writer. Returns true only when
// Resend accepted it, so the caller can report per-recipient results.
async function sendPromoWelcome(row) {
  const apiKey = process.env.RESEND_API_KEY;
  ...
  try {
    const resp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [row.email],
        reply_to: NOTIFY_TO,
        subject: "تم استلام طلب تقييمك الاستثنائي · Scene One",
        html: emailDocument(promoEmail(row)),
      }),
    });
    if (!resp.ok) {
      console.error("Promo email failed:", resp.status, await resp.text());
      return false;
    }
    return true;
  } catch (err) {
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: emailDocument(paymentEmail(row, paymentUrl)),
      }),
    });
  } catch (err) {
...
// Broadcast to every reader (lead + senior + junior) once a script is
...
async function sendReaderNotice(supabaseUrl, serviceKey) {
  ...
  try {
    const resp = await fetch(
      supabaseUrl + "/rest/v1/admins?role=in.(lead_reader,senior_reader,junior_reader)&select=email",
  ...
  // TEST OVERRIDE — set READER_NOTICE_TEST_TO to a single address and the notice
  // goes ONLY there; no real reader is emailed. The lookup above still runs, so a
  // sandbox run still proves the query finds the right people (the count is
  // logged) without spamming them.
  //
  // Set it at **Preview scope only** in Vercel. Production must never have it:
  // with it set, real readers silently stop being notified of new scripts.
  const testTo = (process.env.READER_NOTICE_TEST_TO || "").trim();
  if (testTo) {
    console.warn(
      "Reader notice: TEST MODE (READER_NOTICE_TEST_TO set) — lookup found " +
      emails.length + " reader(s); suppressing them and sending only to " + testTo
    );
  }
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify(Object.assign({
        from: NOTIFY_FROM,
        reply_to: NOTIFY_TO,
        subject: (testTo ? "[TEST] " : "") + "تكليف جديد متاح · Scene One",
        html: emailDocument(readerNoticeEmail()),
      }, testTo
        // Single recipient, and crucially NO bcc — the real reader list is
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: emailDocument(unreconciledEmail(info)),
      }),
    });
  } catch (err) {
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [NOTIFY_TO],
        subject: subject,
        html: emailDocument(refundEmail(row, info)),
      }),
    });
  } catch (err) {
