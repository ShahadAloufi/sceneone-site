...
// transparent rounded corners (48px padding, 32px radius — the same treatment
// the previous wide lockup carried), and it behaves well under every mode:
...
// NOTE the file: this is the EMAIL-ONLY artwork, not either site logo. The site
// files are transparent — and the nav's is now the bare mark with no wordmark at
// all — while this one keeps the wordmark and has the white plate baked in,
// which is the whole point above. The URL is absolute and hardcoded to production — an email
...
// VERSIONED FILENAME, and it has to stay that way. An email is immutable once
// it is in an inbox, but the image it points at is not: replacing the file
// behind a stable URL silently re-renders every email ever sent. That is not
// theoretical — the stacked artwork first shipped over the old name and every
// delivered email redrew it inside the width="185" height="77" its own HTML
// hardcodes, stretching a 551x635 portrait across a landscape box.
//
// So scene-one-email-logo.png stays exactly as it was, serving the archive, and
// new artwork gets a new name. If this changes shape again, add -v3; do not
// overwrite either file.
var LOGO_URL = "https://sceneone.info/assets/scene-one-email-logo-v2.png";

// 120x138 holds the 551x635 plate's aspect ratio and puts the LOCKUP INSIDE it at
// about 99px wide. The artwork is stacked now — the mark over سين ون — where it
// used to be a wide one-line lockup, so the header is taller and narrower than
// the 185x77 it replaced. Both dimensions are set as HTML attributes as well as
// CSS because Outlook ignores the CSS ones and will otherwise draw the image at
// its full 551px width.
var LOGO_W = 120;
var LOGO_H = 138;
...
// Wrap a template's body in a real document that declares itself LIGHT-ONLY.
//
// Every template here returns a bare <div>. With no <head>, a client is never
// told what colour scheme the email supports, so it is free to invent one — and
// the aggressive ones do: they rewrite the card to near-black and, where they
// also invert images, flip the black lockup to white. The result is legible but
// it is not the brand.
//
// These four declarations are the standard way to opt out. `color-scheme` and
// `supported-color-schemes` as meta AND as CSS, because clients read one or the
// other: Apple Mail and iOS Mail honour them and leave the email alone, which
// covers most of where this was going wrong.
//
// What they do NOT cover, and nothing in HTML does: Outlook's Windows clients
// and the Gmail Android app force their own treatment regardless. The logo's
// white plate is what keeps those readable (see the note above), so the two
// defences are complementary — don't drop one because the other exists.
function emailDocument(inner) {
  return "<!doctype html><html><head>" +
    ...
    "</head><body style=\"margin:0;padding:0;background:#f5f1e9;\">" +
    inner +
    "</body></html>";
}

module.exports = { brandHeader, LOGO_URL, emailDocument };
