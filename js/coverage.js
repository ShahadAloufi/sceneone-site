...
(function () {
  ...
  var UI = {
    en: {
      ...
      noteCount: function (n) { return n.toLocaleString("en") + " characters"; },
    ...
    ar: {
      ...
      noteCount: function (n) { return n.toLocaleString("en") + " حرفًا"; },
      // Lead readers deliver their own coverage themselves — it skips quality review.
  ...
  // There is no per-note length limit — see the long comment in
  // /api/review-coverage. The counter below is informational only: it tells the
  // reviewer how much they have written, never how much they have left.
  var noteSaveT = null;
  ...
  // A plain count of what has been written. No ceiling is shown because none
  // applies — it exists so a reviewer can see the weight of a long note, not to
  // ration it.
  function updateNoteCount(els) {
    if (!els.count) return;
    var n = (els.ta.value || "").trim().length;
    els.count.textContent = n ? UI[UILANG].noteCount(n) : "";
